...
class Home(View):
	...
	posts   = blogdata.objects.order_by('-id')
	template_name = 'home.html'
	
	def get(self,request):

		#response = HttpResponse(self, content_type='application/vnd.ms-excel')
		...
		return render(request,'frontend/'+self.template_name,context)


class My_posts(Home):

	user_id  = 8
	posts    = blogdata.objects.filter(author_id=user_id).order_by('-id')
	...
	# return render(request,'frontend/my_posts.html',context)


class My_favorites(Home):
	user_id  = 8
	posts    = blogdata.objects.filter(likes_dislikes__user_id=user_id,likes_dislikes__like_status=1).order_by('-id')
	...
	# return render(request,'frontend/my_favorites.html',context)
...
def testing(request):
	...
	variable = User.objects.filter(blogdata__title__contains='cycle').filter(blogdata__author_id=7)

	variable1 = User.objects.filter(blogdata__title__contains='cycle',blogdata__author_id=7)
